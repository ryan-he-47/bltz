...
(2026-09-13 注:D-1 一票否决已废止;POC 阶段 Gates 仅作趋势检查点。)
...
- **Gate G2**:BPB 对比 + NFE/字节对比 + 完整报告 → **用户拍板 go/no-go** 进入熵臂与 1B。
  **挑战项判定口径**(D1/D2/D5 是否 work,对应 design §1 定位):D-1 逐 Δ 单调通过
  (~~硬指标,一票否决~~ 已废止 2026-09-13,改定性描述);D-7 自验证一致率不显示熵停止无法收容的边际混合;BPB 相对
  token 基线的差距不集中于歧义 patch(R5)。任一失败 → 走 §6 退路,并把失败本身
  写成结论(BLT 组件必要性未被证伪)。
...
- **退场叙事(用户立场 2026-09-08)**:若挑战项(D1/D2/D5)失败,项目不是零产出——
  交付物转为:条件 MTP 解码头作为可迁移组件、字节 I/O 工程结论(哪些便宜 trick
  有效/无效)、逐 Δ 与边界一致性诊断方法论,以及"BLT 组件必要性"的负面证据。
  报告里如实写,不修饰。
