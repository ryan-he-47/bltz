...
- **脚本**(`slurm/`,随仓库同步):bltz_cache.sbatch(CPU,**tiny 分区**——
  ...
  2026-09-15 确诊、未解,见 `docs/29`;fp16
...
- **起飞前体检**(秒级只读,见 `docs/29` §5):`sinfo -N -o '%N|%C|%O|%G|%t'`——
...
- **监控**:`ssh -p 22 yihe47@burgundy.hpc.cityu.edu.hk "tail -20
  ...
  `squeue` 状态**——挂死的 job 与健康长跑在 `sacct` 里完全一样(`docs/29` §4)。
...
- **集群**:VPN 掉线=停手待命;坏节点已排除(`gpu-v100s-06` **驱动层整机卡死**、
  2026-09-15 确诊未解,见 `docs/29`;`gpu-v100s-05` 自 2026-09-04 起 DOWN+DRAIN);
  登录节点不跑重活。
