...
- **当前方向:参数量精调**(架构已验证;M5 连续 Δ 编码已废止——"字符序列
  建模不需要连续性";精确参数解剖与旋钮清单在 **`docs/21`**;精调目标
  待定:纯效率 vs 参数重分配)。
- **进行中:FiLM 条件头实验**(job 555728,OOM 修复后断点续跑中,修复机制
  见 docs/22 §4.1);**T1' 已落地(docs/25):encoder 长度分组重打包**——
  长度分组贴身画布、数值逐元等价(test_enc_grouped 对拍 max diff ~1e-6)、
  纯 torch 零新依赖,本机对拍**全步 -50%**、峰值 -16%;T3.3 分桶被其吸收。
  打包拍板史:T3 实测 docs/24 §5(画布 padding 62%,encoder 占前向
  76-81%)。测试 11 个全绿。
