...
- **当前方向:参数量精调**(架构已验证;M5 连续 Δ 编码已废止——"字符序列
  ...
  待定:纯效率 vs 参数重分配)。
- **进行中:encoder 保守效率砍已入配置**(inducing 16→8、enc_heads 8→4,
  2026-09-14 拍板);**FiLM 条件头实验**(`docs/22`:FiLM 调制注入替代输入
  拼接,RMSNorm+零初始化恒等起步基建,**单臂 20k job 555577 跑集群中**,
  拿 60k 初版当对照,结果回填 docs/22 §4)。测试 10 个全绿。
