...
- **项目已封存(用户拍板,2026-09-14):停止研发,归档保留。** 三大主张被
  外部工作独立覆盖(HAT 2501.10322 / Fast BLT-D 2605.08044 / DMBP
  2608.15454),裁决依据 `docs/13`-`docs/16`,封存说明 **`docs/17`**,
  设计回顾 **`docs/18-retrospective.md`**,60k 收尾手册 **`docs/19`**(待用户通知训练结束后执行)。
  英文入口 `ARCHITECTURE.md`;MIT 协议开源。`docs/` 自此冻结,仅勘误可改。
- **执行史**:设计定稿(09-08)→ 管线+冒烟 → 集群 POC(09-12/13,run 544172
  20k 完成,诊断档案 docs/10)→ gcos 修正测量(docs/11/12,"冲突"判测量
  伪影)→ 60k 长 run 起跑(job 547403)→ 文献普查判 novelty 缺口闭合 → 封存。
- **唯一在途事项**:60k 收尾(按 docs/19;之后集群侧数据/缓存/中间 ckpt 全清,
  home 只留源码/日志/final+best;不动用户其它资产)。集群 burgundy.hpc.cityu.edu.hk:22。**home
