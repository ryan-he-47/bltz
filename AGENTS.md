...
- 悬置:多语言 typo 模式备录;下游任务级评估未建(docs/34 §7 缺口清单)。
  **2026-09-24 拍板关闭:v1 全部封存不再续(boundary_2b 主 run 弃),
  48-plain 解混臂取消(sbatch 存档不挂);typo-2B 配方(48d typo-AE +
  MDN 骨干 + 增强 BPE)定为默认配置,后续一切基于此。**
- **2026-09-24 教训:v2 时代 batch=16 是 v1 遗产未重标定**——v1 逐 Δ
  查询网格+FiLM 头显存紧张时定的 16;v2 实测(最重的 swiglu 臂,317M
  参数,grad_ckpt)batch=16 峰值仅 ~8GiB,V100 32GB 余量 4×。**后续
  新跑前按"实测峰值 ×2 余量"重标定 batch,别再继承。**
...
- **2026-09-17 用户裁决:H1 不成立,v1 架构存在原理性缺陷**——条件独立
  ...
  **AE 验收口径(2026-09-19 拍板)**:POC 起跑前三件套——几何套件/
  ...
  (无逐Δ查询网格,比 v1 快 2.7×,60k 全程 ≈7h)。**60k POC 已完成并首读
  (2026-09-19,docs/32 §S3)**:NLL -32.8 衰减尾段仍在上探(早收敛是假象),
  零泛化间隙;argmax 精确命中 32.7%/分量覆盖 60.9%/π 无坍缩;分锅探针判
  **解码器无辜、误差在骨干预测**(抗噪 0.07σ→EM 99.5%);幽灵词病理在
  原理层根治(采样=词级温度);瓶颈=训练量+排序锐度。**推理侧重排免费
  +3pp EM**(min-σ ≈ 解码器置信;top5 候选含真值 48.5%);失败模式以
  错别字级为主(edit≤2 占 53%)。**1B 长训 575524 运行中**(2026-09-20:
  50k 里程碑分叉,LR 2e-4 对齐,122070 步 ≈1.0B patch,~8h)。
