...
Query evaluation is chunked (loss.query_chunk) to bound memory — same trick as
MoB_Head's --loss-chunk. Gradients flow through the chunk loop unchanged.
"""
...
def mtp_loss(model, batch: dict[str, torch.Tensor], cfg) -> torch.Tensor:
    ...
    h = model(byte_ids, batch["pad_mask"])[:, : S - 1]  # (B, S-1, d)
    ...
    # target byte for query (j, delta): flat[ends[j] + delta], delta 0-based
    tpos = ends[:, : S - 1, None] + delta[None, None, :]  # (B, S-1, k_max)
    valid = delta[None, None, :] < K_j[:, :, None]  # delta in [0, K_j)
    ...
    k = (qpatch - j_idx[None, :, None]).float()  # 1..n where valid
    w = torch.where(valid, lam ** (k - 1.0), torch.zeros(B, S - 1, k_max, device=dev))

    b_all = (
        torch.arange(B, device=dev).view(B, 1, 1).expand(B, S - 1, k_max).reshape(-1)
    )
    j_all = j_idx.view(1, S - 1, 1).expand(B, S - 1, k_max).reshape(-1)
    d_all = delta.view(1, 1, k_max).expand(B, S - 1, k_max).reshape(-1)
